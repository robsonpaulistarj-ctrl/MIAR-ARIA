...
Conduct a focused discovery conversation before building anything. Use the `AskQuestion` model tool for each round — do not ask questions via plain text.

Question type guidance for `AskQuestion`:

- **Open-ended questions** (name, description, details): send `{ "question": "..." }` only — no `choices` field. This renders as a free text input.
- **Multiple-choice questions** (style, features, monetization): include `choices` with a list of choices. Always add "Something else" as the last choice.

Example `AskQuestion` call:

```json
{
  "question": "How will it make money?",
  "choices": ["Subscription", "One-time purchase", "Freemium", "Ads", "Something else"]
}
```
